...
#
# Usage:
#	./create_cesm_run_control_nudge_long.csh
#****************************************************************

## Set option variables in this section
# Set case parameters & name
set CASETITLE=control_test_nudge_long
set RES=f09_f09_mg17
set COMPSET=F1850
set COMPSET_SHORT=f

set REFCASE=${COMPSET_SHORT}.e22.${COMPSET}.${RES}.control_test.002
...
foreach i ( 1 2 3 )
	...
	if ($CONT_RUN == FALSE) then
		...
		# Runtime variables
		./xmlchange STOP_OPTION="nyears",RESUBMIT=0,STOP_N=3,JOB_WALLCLOCK_TIME=5:57:00,REST_N=3,REST_OPTION="nyears",CONTINUE_RUN=$CONT_RUN
...
mfilt = 1
empty_htapes = .true.
fincl1 = 'CLDLIQ','CLDICE','T','PS','CLOUD','IWC','LWC','EMIS','TOT_CLD_VISTAU','TS','TGCLDCWP','TGCLDLWP','TGCLDIWP','FLDS','FSDS','LWCF','SWCF','PRECL','PRECSL','FLUT','FSUTOA','U','V','Target_U','Target_V','Target_T','OPTS_MAT'
inithist = 'MONTHLY'
Nudge_Model = .true.
...
EOFcam
	...
	if ($CONT_RUN == TRUE) then
		...
		./xmlchange JOB_QUEUE="regular",JOB_WALLCLOCK_TIME=5:57:00
		./xmlquery JOB_QUEUE,JOB_WALLCLOCK_TIME
			
		# Do XMLCHANGE options for CONTINUE_RUN - 36 years
		./xmlquery STOP_N,STOP_OPTION,RESUBMIT,REST_N,CONTINUE_RUN
		./xmlchange STOP_N=3,STOP_OPTION="nyears",RESUBMIT=11,REST_N=3,REST_OPTION="nyears",CONTINUE_RUN=$CONT_RUN
